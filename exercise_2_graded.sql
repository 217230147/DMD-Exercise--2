...
WITH ranked_treatments AS (
    SELECT t.*, p.name AS patient_name,
            ROW_NUMBER() OVER (PARTITION BY t.patient_id ORDER BY t.treatment_time DESC) AS rn
    FROM treatments t
    JOIN patients p ON t.patient_id = p.patient_id
)
SELECT patient_id, patient_name, treatment_id, treatment_type, treatment_time
...
WITH patient_max_age AS (
    SELECT d.specialization, d.doctor_id, d.name AS doctor_name,
...
SELECT pma.specialization, pma.doctor_name, pma.max_patient_age
FROM patient_max_age pma
JOIN specialization_max_age sma
    ON pma.specialization = sma.specialization
    AND pma.max_patient_age = sma.overall_max_age;
